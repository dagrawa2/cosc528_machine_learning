...
model.train(Z, eps=10**-9, sigma_offset=10**-1)

clusters = model.cluster_hard(Z)
cluster_sizes = np.array([len(clusters[clusters==k]) for k in range(model.K)])
file = open("Clusters\\GMM\\Z2D\\output.txt", "w")
file.write("K = "+str(K)+"\r\n\r\n")
...
for k in range(K):
	...
	L.sort()
	file = open("Clusters\\GMM\\Z2D\\cluster_"+str(k+1)+".txt", "w")
...
fig.savefig("Clusters\\GMM\\Z2D\\plot.png", bbox_inches='tight')
#plt.show()
